    ...
    <div class="main">
        ...
        <div class="center_box cb">
            ...
            <div class="page_content bg_gray">
                ...
                <div class="uo_body">
                    <div class="wrapper">
                        @include('layout.front.message')
                        <div class="uofb cf">
                            <div class="l_col adrs">
                                <h2>Add New Address</h2>
                                {{ Html::ul($errors->all()) }}
                                {{ Form::open(['url' => '/', 'files' => true, 'id' => 'facade', 'method' => 'post']) }}
                                <div class="field">
                                    {{ Form::label('Name', 'Name *') }}
                                    {{ Form::text('name', '', ['class' => 'vl_empty', 'placeholder' => '']) }}
                                </div>

                                <div class="field">
                                    {{ Form::label('City', 'Your city *') }}
                                    {{ Form::select('city', $cities, '', ['class' => 'vl_empty' ]) }}
                                </div>

                                <div class="field">
                                    {{ Form::label('Area', 'Your area *') }}
                                    {{ Form::select('area', $areas, '', ['class' => 'vl_empty' ]) }}
                                </div>
                                <div class="field">
                                    {{ Form::label('Street', 'Street') }}
                                    {{ Form::text('street', '', ['class' => 'vl_empty', 'placeholder' => '']) }}
                                </div>
                                <div class="field">
                                    {{ Form::label('House', 'House #') }}
                                    {{ Form::text('house', '', ['class' => 'vl_empty', 'placeholder' => 'House Name / Number']) }}
                                </div>
                                <div class="field">
                                    {{ Form::label('Additional information', 'Additional information', ["class" => 'pos_top']) }}
                                    {{ Form::textarea('additional') }}
                                </div>
                                {{ Form::submit('add address', ['class' => 'green_btn']) }}
                                {{ Form::close() }}
                            </div>
                            <div class="r_col">
                                <h2>My Addresses</h2>
                                ...
                                <div class="uo_adr_list">
                                    ...
                                    @foreach($address as $item)
                                        <div class="item">
                                            ...
                                            <div class="actbox">
                                                <a href="{{ route('address.delete', ['id' => $item->id]) }}"
                                                   class="bcross"></a>
                                            </div>
                                        </div>
                                    @endforeach
